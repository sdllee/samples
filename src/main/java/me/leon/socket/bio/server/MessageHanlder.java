...
public class MessageHanlder implements Runnable {

    private static Logger logger = LoggerFactory.getLogger(MessageHanlder.class);

    private Socket incoming; // 收到的链接

    public MessageHanlder(Socket incoming) {
    ...
    public void run() {
        ...
        try {
            ...
            try {
                outStream.write(returnMessage.getBytes("UTF-8"));
            } catch (IOException e) {
                e.printStackTrace();
            }
